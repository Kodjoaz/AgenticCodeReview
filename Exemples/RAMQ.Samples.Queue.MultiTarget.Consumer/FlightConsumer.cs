...
{
    public class FlightConsumer : BaseConsumer<FlightMessage>
    {
        ...
        public override async Task<MessageTransitContext<MessageTransitResponse>> ConsumeAsync(
            MessageTransitContext<FlightMessage> context,
            CancellationToken cancellationToken)
        {
            ...
            {
                Logger.LogWarning("Annulation en cours FlightConsumer MessageId={MessageId}", context.MessageId);
                throw;
            }
            catch (Exception ex)
            {
                ...
                reply.Content = ex.Message;
                reply.IsTransient = true;
                Logger.LogError(ex, "Erreur FlightConsumer MessageId={MessageId}", context.MessageId);
