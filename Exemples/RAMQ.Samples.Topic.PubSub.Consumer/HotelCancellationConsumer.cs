...
{
    public class HotelCancellationConsumer : BaseConsumer<NotifyEvent>
    {
        public HotelCancellationConsumer(
            IMessagingProvider messagingProvider,
            ILogger<HotelCancellationConsumer> logger,
            ...
            CancellationToken cancellationToken)
        {
            ...
            var reply = new MessageTransitResponse
            {
                ...
                Content = $"HotelCancellation démarré (ReservationId={context.Message.ReservationId})"
            };

            try
            {
                Logger.LogInformation("HotelCancellationConsumer start MessageId={MessageId} ReservationId={ReservationId}",
                    context.MessageId, context.Message!.ReservationId);

                await Task.Delay(300, cancellationToken);

                if (!string.IsNullOrWhiteSpace(context.Message.Content) &&
                    context.Message.Content.Contains("hotelfailed", StringComparison.OrdinalIgnoreCase))
                    throw new ExponentialRetryException("Échec transitoire hotel cancellation", statusCode: 503);

                await CompleteMessageAsync(cancellationToken);

                reply.Content = $"HotelCancellation complété (ReservationId={context.Message.ReservationId})";
                Logger.LogInformation("HotelCancellationConsumer success MessageId={MessageId} ReservationId={ReservationId}",
                    context.MessageId, context.Message.ReservationId);
            }
            ...
            {
                Logger.LogWarning("Annulation en cours HotelCancellationConsumer MessageId={MessageId}", context.MessageId);
                throw;
            }
            catch (Exception ex)
            {
                reply.StatusCode = (int)System.Net.HttpStatusCode.InternalServerError;
                reply.Content = $"Erreur HotelCancellationConsumer: {ex.Message}";
                reply.IsTransient = true;
                Logger.LogError(ex, "Erreur général HotelCancellationConsumer MessageId={MessageId}", context.MessageId);
