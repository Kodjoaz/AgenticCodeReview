...
{
    public class HotelBookingConsumer : BaseConsumer<NotifyEvent>
    {
        public HotelBookingConsumer(
            IMessagingProvider messagingProvider,
            ILogger<HotelBookingConsumer> logger,
            ...
            CancellationToken cancellationToken)
        {
            ...
            var reply = new MessageTransitResponse
            {
                ...
                Content = $"HotelBooking démarré (ReservationId={context.Message.ReservationId})"
            };

            try
            {
                Logger.LogInformation("HotelBookingConsumer start MessageId={MessageId} ReservationId={ReservationId}",
                ...
                if (!string.IsNullOrWhiteSpace(context.Message.Content) &&
                    ...
                    throw new ExponentialRetryException("Échec transitoire hotel booking", statusCode: 503);

                await CompleteMessageAsync(cancellationToken);

                reply.Content = $"HotelBooking complété (ReservationId={context.Message.ReservationId})";
                Logger.LogInformation("HotelBookingConsumer success MessageId={MessageId} ReservationId={ReservationId}",
                    context.MessageId, context.Message.ReservationId);
            }
            ...
            {
                Logger.LogWarning("Annulation en cours HotelBookingConsumer MessageId={MessageId}", context.MessageId);
                throw;
            }
            catch (Exception ex)
            {
                reply.StatusCode = (int)System.Net.HttpStatusCode.InternalServerError;
                reply.Content = $"Erreur HotelBookingConsumer: {ex.Message}";
                reply.IsTransient = true;
                Logger.LogError(ex, "Erreur général HotelBookingConsumer MessageId={MessageId}", context.MessageId);
