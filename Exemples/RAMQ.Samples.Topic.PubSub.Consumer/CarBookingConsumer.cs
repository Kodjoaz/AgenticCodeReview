...
{
    public class CarBookingConsumer : BaseConsumer<NotifyEvent>
    {
        public CarBookingConsumer(
            IMessagingProvider messagingProvider,
            ILogger<CarBookingConsumer> logger,
            ...
            CancellationToken cancellationToken)
        {
            ...
            var reply = new MessageTransitResponse
            {
                ...
                Content = $"CarBooking démarré (ReservationId={context.Message.ReservationId})"
            };

            try
            {
                Logger.LogInformation("CarBookingConsumer start MessageId={MessageId} ReservationId={ReservationId}",
                    context.MessageId, context.Message!.ReservationId);

                await Task.Delay(300, cancellationToken);

                if (!string.IsNullOrWhiteSpace(context.Message.Content) &&
                    context.Message.Content.Contains("carfailed", StringComparison.OrdinalIgnoreCase))
                    throw new ExponentialRetryException("Échec transitoire car booking", statusCode: 503);

                await CompleteMessageAsync(cancellationToken);

                reply.StatusCode = (int)System.Net.HttpStatusCode.Accepted;
                reply.Content = $"CarBooking complété (ReservationId={context.Message.ReservationId})";

                Logger.LogInformation("CarBookingConsumer success MessageId={MessageId} ReservationId={ReservationId}",
                    context.MessageId, context.Message.ReservationId);
            }
            ...
            {
                Logger.LogWarning("Annulation en cours CarBookingConsumer MessageId={MessageId}", context.MessageId);
                throw;
            }
            catch (Exception ex)
            {
                reply.StatusCode = (int)System.Net.HttpStatusCode.InternalServerError;
                reply.Content = $"Erreur CarBookingConsumer: {ex.Message}";
                reply.IsTransient = true;
                Logger.LogError(ex, "Erreur général CarBookingConsumer MessageId={MessageId}", context.MessageId);
