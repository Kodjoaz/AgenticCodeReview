...
{
    public class CarCancellationConsumer : BaseConsumer<NotifyEvent>
    {
        public CarCancellationConsumer(
            IMessagingProvider messagingProvider,
            ILogger<CarCancellationConsumer> logger,
            ...
            CancellationToken cancellationToken)
        {
            ...
            var reply = new MessageTransitResponse
            {
                ...
                Content = $"CarCancellation démarré (ReservationId={context.Message.ReservationId})"
            };

            try
            {
                Logger.LogInformation("CarCancellationConsumer start MessageId={MessageId} ReservationId={ReservationId}",
                    context.MessageId, context.Message!.ReservationId);

                await Task.Delay(250, cancellationToken);

                if (!string.IsNullOrWhiteSpace(context.Message.Content) &&
                    context.Message.Content.Contains("cancellationfailed", StringComparison.OrdinalIgnoreCase))
                    throw new ExponentialRetryException("Échec transitoire car cancellation", statusCode: 503);

                await CompleteMessageAsync(cancellationToken);

                reply.StatusCode = (int)System.Net.HttpStatusCode.Accepted;
                reply.Content = $"CarCancellation complété (ReservationId={context.Message.ReservationId})";

                Logger.LogInformation("CarCancellationConsumer success MessageId={MessageId} ReservationId={ReservationId}",
                    context.MessageId, context.Message.ReservationId);
            }
            ...
            {
                Logger.LogWarning("Annulation en cours CarCancellationConsumer MessageId={MessageId}", context.MessageId);
                throw;
            }
            catch (Exception ex)
            {
                reply.StatusCode = (int)System.Net.HttpStatusCode.InternalServerError;
                reply.Content = $"Erreur CarCancellationConsumer: {ex.Message}";
                reply.IsTransient = true;
                Logger.LogError(ex, "Erreur général CarCancellationConsumer MessageId={MessageId}", context.MessageId);
