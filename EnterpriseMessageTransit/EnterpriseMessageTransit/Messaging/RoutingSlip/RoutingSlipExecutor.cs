...
namespace RAMQ.COM.EnterpriseMessageTransit.Messaging.RoutingSlip
{
    ...
    internal sealed class RoutingSlipExecutor<TArgs> : IRoutingSlipExecutor where TArgs : class
    {
        ...
        private async Task RunAsync(IMessagingProvider provider, CancellationToken ct)
        {
            ...
            // Synchroniser Attempt avec DeliveryCount du message Service Bus (1-based)
            if (ctx.TransportMessage != null)
            {
                ctx.Attempt = ctx.TransportMessage.DeliveryCount;
            }
            else if (ctx.Attempt == 0)
            {
                ctx.Attempt = 1; // fallback pour compatibilité
            }
            ...
            switch (activityResult)
            {
                ...
                case ActivityResult.RetryImmediateResult retryImmediate:
                    ...
                    stepActivity?.SetStatus(ActivityStatusCode.Error, retryImmediate.Reason);
                    await provider.ImmediateRetryAsync(new ImmediateRetryException(retryImmediate.Reason), ct);
                    break;
                ...
                case ActivityResult.RetryExponentialResult retryExponential:
                    ...
                    stepActivity?.SetStatus(ActivityStatusCode.Error, retryExponential.Reason);
                    var retryException = retryExponential.InnerException is null
                        ? new ExponentialRetryException(retryExponential.Reason)
                        : new ExponentialRetryException(retryExponential.Reason, retryExponential.InnerException);
                    await provider.ExponentialRetryAsync(retryException, ct);
                    break;
