...
#### 3.2.4 Cible architecturale — Routing Slip natif avec breaking change MAJOR

Avec les breaking changes assumés, la cible s'aligne directement sur la proposition complète de `docs/architecture-routing-slip.md`. Le principe central : **l'itinéraire voyage avec le message** (`SlipEnvelope`) et **le code métier d'une étape ne connaît plus le framework**.

```
┌──────────────────────────────────────────────────────┐
│  ACTIVATEUR (Azure Function HTTP)                    │
│  Construit le slip via RoutingSlipBuilder             │
│  et publie vers la première queue                    │
└────────────────────────┬─────────────────────────────┘
                         │ IMessageProducer<SlipEnvelope>
                         ▼
┌──────────────────────────────────────────────────────┐
│  ROUTING SLIP EXECUTOR (interne EMT)                 │
│  RoutingSlipExecutor<TArgs>                          │
│  Désérialise SlipEnvelope, résout IRoutingSlipActivity│
│  via DI, avance le curseur, publie au suivant        │
│  Aucune dépendance dans votre code métier            │
└────────────────────────┬─────────────────────────────┘
                         │ appelle
                         ▼
┌──────────────────────────────────────────────────────┐
│  ACTIVITÉ (votre code métier)                        │
│  IRoutingSlipActivity<TArgs>                         │
│  Reçoit ActivityContext<TArgs>, retourne ActivityResult│
│  Ne sait PAS qu'elle est dans un routing slip        │
│  Testable en isolation : zéro dépendance EMT         │
└──────────────────────────────────────────────────────┘
```

**Ce qui est supprimé (breaking change v2.0) :**

| Supprimé | Remplacé par |
|---|---|
| `BaseConsumer<T>.RouteToNextStageAsync()` | `IRoutingSlipActivity<TArgs>` + `RoutingSlipExecutor` |
| `AppSettings.Itinerary` multi-étapes | `RoutingSlipBuilder` côté activateur |
| `MessageTransitContext.CurrentStage` (saga) | `SlipEnvelope.Cursor` |
| `Variables["__FinalStageCompleted"]` | Détection automatique `Cursor == Steps.Length` |
| `FindIndexFromStage` (3 stratégies) | Curseur entier — indexation O(1), sans ambiguïté |
| `AdvancingConsumer<T>` (intermédiaire prévu) | Directement `IRoutingSlipActivity<TArgs>` |

**Ce qui reste inchangé pour les consumers simples (non-saga) :**
- `BaseConsumer<T>` avec settlement uniquement (`CompleteMessageAsync`, `DeadLetterMessageAsync`)
- `IMessageProducer<T>`, `IMessageTransit`, `IMessagingProvider`
- Claim-check, circuit breaker, journal, retry — tous transparents

**Points clés du nouveau design :**

1. **`SlipEnvelope` est le porteur d'état unique** — l'itinéraire complet + curseur + variables voyagent avec le message. Plus de dérive de config entre applications.

2. **`IRoutingSlipActivity<TArgs>`** — interface que le développeur implémente. Reçoit `ActivityContext<TArgs>` (arguments + variables partagées + SlipId). Retourne `ActivityResult` (`Next()`, `Complete()`, `Fault(ex)`). **Zéro dépendance EMT** : testable avec un simple `new`.

3. **`RoutingSlipBuilder`** — l'activateur construit l'itinéraire explicitement (`AddStep<TArgs>`, `AddStepTopic<TArgs>`). Chaque paramètre est typé, validé à la compilation. Fini les configs JSON mal formées découvertes en production.

4. **`RoutingSlipExecutor<TArgs>`** (internal) — résout l'activité via DI, avance le curseur, publie. Le consumer applicatif n'appelle jamais cette classe.

5. **`BaseConsumer<T>` subsiste** pour les consumers non-saga (traitement point-à-point simple). Il perd `RouteToNextStageAsync` et toute la mécanique saga — il n'est plus une god-class.

#### 3.2.5 Plan de migration en 5 étapes (pour un développeur junior)

Cette refonte est un **breaking change MAJOR (v2.0)**. L'objectif n'est plus de préserver la compatibilité binaire, mais de migrer proprement avec un guide clair et une coordination avec les équipes consommatrices.

| Étape | Action | Risque |
|---|---|---|
| **E1** | **Créer le dossier `Messaging/RoutingSlip/`** dans l'assembly courant. Implémenter `SlipEnvelope`, `SlipStep`, `SlipHeader`, `ActivityResult`, `ActivityContext<TArgs>`, `IRoutingSlipActivity<TArgs>`, `RoutingSlipBuilder`, `RoutingSlipExecutor<TArgs>`. | Faible — code additif, aucune suppression. |
| **E2** | **Écrire les tests unitaires** du `RoutingSlipExecutor` : avancement curseur, détection fin de slip, `Fault` → DLQ, propagation des variables, idempotence (step reçu 2 fois). Pas de Service Bus réel — utiliser `InMemoryMessagingProvider`. | Nul — tests purs. |
| **E3** | **Enregistrer les extensions DI** (`AddRoutingSlipActivity`, `AddRoutingSlipActivityForTopic`) et les Azure Function triggers correspondants. Valider de bout-en-bout sur le Service Bus Emulator (T10). | Moyen — premier contact avec le broker. |
| **E4** | **Supprimer de `BaseConsumer<T>`** : `RouteToNextStageAsync`, `FindIndexFromStage`, `ResolveEffectiveCurrentStage`, la dépendance à `AppSettings.Itinerary` multi-étapes, et le flag `__FinalStageCompleted`. Supprimer les propriétés saga de `AppSettings`. Bumper la version en **v2.0**. Publier le guide de migration (`architecture-routing-slip.md §13`). | Élevé — breaking change visible. À coordonner avec toutes les équipes consommatrices avant le tag. |
| **E5** | **Nettoyage final** : enrichir `IMessageTransit` (`ApplicationProperties`, `DeliveryCount`, `EnqueuedTimeUtc`, `CorrelationId`, `ReplyTo`), rendre `RawMessage` `internal`, mettre à jour `PublicAPI.Shipped.txt` / `PublicAPI.Unshipped.txt`. Préparer le dossier `RoutingSlip/` pour la scission en assembly (Phase 6). | Faible si E1-E4 sont validés et les équipes ont migré. |

💡 **Pour un junior — la règle d'or de ce breaking change.** *« Un breaking change assumé est moins dangereux qu'un refactor silencieux. »* La casse est **visible** (erreur de compilation chez les consommateurs), **documentée** (guide de migration §13), et **coordonnée** (tag v2.0 annoncé). Un refactor silencieux qui change le comportement runtime sans erreur de compilation est autrement plus difficile à déboguer.
...
**Coût :** moyen à élevé — breaking change MAJOR pour les applications consommatrices multi-étapes (migration requise vers `IRoutingSlipActivity<TArgs>`), accompagné d'un guide de migration complet. Les consumers simples (non-saga) ne sont pas impactés. Cible : **Phase 5** ([§8.3](#83-plan-progressif-par-phases)).
...
| **Phase 5 — Routing Slip natif (breaking change MAJOR v2.0)** | Implémenter le Routing Slip complet (cf `docs/architecture-routing-slip.md`) avec **breaking changes assumés** sur les consumers multi-étapes existants. Bump de version MAJOR (v1.x → v2.0). | 6-10 semaines | **Risque 🟠 élevé** — breaking change explicite et délibéré : `RouteToNextStageAsync` supprimé, `AppSettings.Itinerary` multi-étapes retiré, `CurrentStage` saga remplacé par `SlipEnvelope.Cursor` ; chaque application consommatrice multi-étapes doit migrer avant d'adopter v2.0. Risque maîtrisable car la casse est **visible et documentée** (contrairement à un refactor silencieux). Atténuation : tag `v2.0` coordonné avec toutes les équipes consommatrices, guide de migration complet (`architecture-routing-slip.md §13`), fenêtre de migration planifiée. **Complexité 🔴 très élevée** — introduction de `SlipEnvelope` comme contrat sérialisé pivot (nécessite Phase 3 préalable), remplacement de `BaseConsumer<T>` saga par `IRoutingSlipActivity<TArgs>` + `RoutingSlipExecutor`, suppression de `FindIndexFromStage` / `__FinalStageCompleted` / `AppSettings.Itinerary` multi-étapes. | `IRoutingSlipActivity<TArgs>`, `ActivityContext<TArgs>`, `ActivityResult` (public) ; `RoutingSlipBuilder`, `SlipEnvelope`, `SlipStep` (public) ; `RoutingSlipExecutor<TArgs>` (internal) ; extensions DI `AddRoutingSlipActivity` / `AddRoutingSlipActivityForTopic` ; `IMessageTransit` enrichi (`ApplicationProperties`, `DeliveryCount`, `EnqueuedTimeUtc`, `CorrelationId`, `ReplyTo`) ; `RawMessage` rendu `internal` ; `RouteToNextStageAsync` **supprimé** de `BaseConsumer<T>` ; `AppSettings.Itinerary` multi-étapes **retiré** ; guide de migration + CHANGELOG v2.0 + `PublicAPI.Unshipped.txt` à jour. | D4 | Toute activité métier testable sans Service Bus (POCO pur, zéro mock EMT) ; aucun cast `IMessageTransit` → `AzureFunctionMessageTransit` ; slip complet exécutable de bout-en-bout sur `InMemoryMessagingProvider`. |
...
- **Phase 5 avant la Phase 6** : c'est en Phase 5 que `IMessageTransit` devient *réellement* portable et que `SlipEnvelope` remplace le couplage sur `AppSettings.Itinerary`. Le breaking change MAJOR v2.0 est **intentionnel** : il nettoie la surface publique avant la scission en assemblies. Scinder (Phase 6) avant ce nettoyage reviendrait à répliquer les anciennes APIs obsolètes dans plusieurs packages.
...
| Phase 5 | (a) `IMessageTransit` couvre-t-il 100 % des besoins (zéro cast `RawMessage`) ? (b) Toutes les équipes consommatrices multi-étapes ont-elles migré vers `IRoutingSlipActivity<TArgs>` ou planifié leur migration ? | Sinon, ne pas lancer la Phase 6 — la portabilité et le nettoyage de surface ne tiendront pas. |
