...
- **`AppSettings`** — Configuration racine. Contient le namespace Service Bus, le nom d'application, le journal (table name + URI), l'itinéraire (`Itinerary`) et la politique de retry. Validable via `IValidatableObject`.
...
| **Itinerary** | Liste ordonnée d'audiences (étapes Saga) |
